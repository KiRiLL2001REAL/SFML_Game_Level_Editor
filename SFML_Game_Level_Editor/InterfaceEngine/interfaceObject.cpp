...
namespace edt
{
	...
	{
		if (state)
		{	// Если нужно взвести бит
			// Применяем результат побитового ИЛИ от "options" и параметра "one_option"
			options |= one_option;
			return;
		}
		// Иначе применяем результат побитового И от "options" и инвертированного параметра "one_option"
		options &= ~one_option;
	}
